...
[
  ...
  ...all ${appsWithCV.length} candidates...
    ...
    const sortedCandidates = rankedCandidates.sort((a, b) => b.agentic_score - a.agentic_score);
    sortedCandidates.forEach((rc, i) => { rc.rank = i + 1; });

    // ═══ AGENTIC STEP 5: Save results with full history ═══
    await Promise.all(sortedCandidates.map(async (rc) => {

...
async function sendEmailWithResend({ to, subject, body, from_name = "QuantaHire" }) {
...
Deno.serve(async (req) => {
  try {
    ...
    // Send email via Resend (test mode: send to your email, CC candidate in subject)
    await sendEmailWithResend({ to: "shahadym0@gmail.com", subject: `[TO: ${data.candidate_email}] ${subject}`, body });

...
Deno.serve(async (req) => {
  try {
    ...
    if (!row && entityId) {
      row = await base44.asServiceRole.entities.RecruiterProfile.get(entityId);
    }
    ...
    await sql`CREATE TABLE IF NOT EXISTS "recruiter_profiles" ("id" TEXT PRIMARY KEY)`;
    const cols = Object.keys(row).filter(k => k !== "id");
    for (const col of cols) {
      await sql(`ALTER TABLE "recruiter_profiles" ADD COLUMN IF NOT EXISTS "${col}" TEXT`);
    }

    if (eventType === "delete") {
      await sql(`DELETE FROM "recruiter_profiles" WHERE "id" = $1`, [row.id]);
    ...
    await sql(
      `INSERT INTO "recruiter_profiles" (${colNames}) VALUES (${placeholders}) ON CONFLICT ("id") DO UPDATE SET ${updateSet}`,
      values
    );
